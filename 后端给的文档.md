...
该接口使用先进的Ark AI视觉模型进行图像分析，能够识别各种农作物种类、生长阶段和健康状况。系统会根据分析结果提供专业的种植管理建议。分析结果包含以下字段：
- `analysisTime`: 分析时间
- `cropType`: 作物种类（如：水稻、小麦、玉米等）
- `growthStage`: 生长阶段（如：抽穗期、开花期等）
- `healthStatus`: 健康状态（如：健康、轻度病害、重度病害等）
- `analysisDetail`: 详细分析说明
- `suggestions`: 针对当前作物状况的管理建议列表
...
```bash
curl -X GET https://gwyixuidazse.sealosbja.site/api/device/data \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```
...
```bash
curl -X POST https://gwyixuidazse.sealosbja.site/api/device/data \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." \
  -d '{"temperature": 25.0, "moisture_vol": 30.0, "ph": 6.5, "ec": 1200}'
```

### 获取设备历史数据

```bash
curl -X GET "https://gwyixuidazse.sealosbja.site/api/device/data/history?page=1&pageSize=10" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
``` 